...
\]
Here, $x \in \mathbb{R}^N$ is a $N$-dimensional feature vector, $w \in
\mathbb{R}^N$ are $N$ weights, $b$ is the bias, $\hat{y} \in [0, 1]$
is the prediction of the model based on $x$, and $y$ is the ground
truth label.
